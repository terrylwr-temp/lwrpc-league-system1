...
import {approvedList,approvedDetail,approvedPreflight,approvedMutation,approvedFormalSource,approvedSourceReview} from '../../../lib/aiApprovedAnswersService.js';
import {existingEvidenceCase,confirmExistingEvidence} from '../../../lib/aiExistingEvidenceDecision.js';
...
async function handle(request){
 try{
  ...
  if(request.method==='GET'){
   ...
   else if(op==='case')result=await existingEvidenceCase(auth,params.get('id'));
   ...
   result=body.action==='existing-evidence'?await confirmExistingEvidence(auth,body):body.action==='preflight'?await approvedPreflight(auth.supabase,body.id,auth.user.id):await approvedMutation(auth.supabase,body,auth.user.id);
  }
