...
export default function ExistingEvidenceDecision({data,request,retest,children}){
 ...
 if(saved)return <section className={styles.decision}><p role="status">Recorded as AI/Retrieval Review. The case remains open. No Approved Answer was created.</p><button onClick={()=>retest(data.question)}>Retest Question</button> <a href={`/ai-assistant/review?group=${encodeURIComponent(data.groupId)}`}>Return to review case</a></section>;
