"use client";
import { activationDisplay } from '../lib/aiDocumentActivation';
...
function DocumentVersionHistory({selected, expanded, onToggle, onSelect}) {
  const active = selected.versions.find(version => version.id === selected.document.active_version_id);
  const prior = selected.versions.filter(version => version.id !== active?.id).slice().sort((a,b) => String(b.created_at || '').localeCompare(String(a.created_at || '')));
  const historyId = `prior-versions-${selected.document.id}`;
  const card = (version, isActive = false) => <button key={version.id} type="button" onClick={() => onSelect(version.id)} className={`flex w-full flex-wrap items-center justify-between gap-3 rounded-xl border px-3 py-3 text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600 ${selected.selectedVersionId === version.id ? 'border-blue-400 bg-blue-50' : 'border-slate-200 bg-white'}`}><span className="min-w-0 flex-1"><strong className="block break-words text-sm text-slate-950">{version.original_filename}</strong><small className="block break-all text-xs font-semibold text-slate-500">{version.version_label} · {version.page_count ?? '-'} pages · {version.chunk_count} chunks</small><small className="mt-1 block text-xs text-slate-600">Activated: {activationDisplay(version).time}<br />Activated by: {activationDisplay(version).actor}</small></span><span className="flex flex-wrap gap-2">{isActive && <Status value="active" />}<Status value={version.processing_status} /></span></button>;
  return <div className="mt-5 border-t border-slate-200 pt-4"><h3 className="text-sm font-black text-slate-950">Versions</h3>{active && <div className="mt-3 space-y-2"><h4 className="text-sm font-black text-emerald-800">Active Version</h4>{card(active, true)}</div>}<button type="button" aria-expanded={expanded} aria-controls={historyId} onClick={onToggle} className="mt-3 flex min-h-11 w-full items-center justify-between gap-3 rounded-xl border border-slate-300 bg-slate-50 px-3 py-3 text-left text-sm font-bold text-slate-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600">Prior Versions ({prior.length}) <span aria-hidden="true">{expanded ? '▴' : '▾'}</span></button><div id={historyId} hidden={!expanded} className="mt-2 space-y-2">{prior.map(version => card(version))}{!prior.length && <p className="text-sm text-slate-500">No prior versions.</p>}</div></div>;
}
