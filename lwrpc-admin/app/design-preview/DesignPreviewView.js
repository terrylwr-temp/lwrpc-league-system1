...
export default function DesignPreviewView({ dashboard = {} }) {
  ...
  const hasStandingsTiebreakDetails = hasStandingsTiebreak(dashboard.standingsLeaders, selectedTeam?.divisions);
        ...
        <div className={styles.modalLayer} role="dialog" aria-modal="true" aria-labelledby="standings-dialog-title">
          ...
          <section className={styles.standingsDialog}>
            ...
            <div className={styles.standingsChart}>
              {hasStandingsTiebreakDetails && <button type="button" onClick={() => setTiebreakDetailsOpen((current) => !current)} className="mb-3 rounded-lg border border-emerald-300 bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-950 hover:bg-emerald-100" aria-expanded={tiebreakDetailsOpen}>
                {tiebreakDetailsOpen ? "Hide Tiebreak Details" : "Tiebreak Details"}
              </button>}
              {hasStandingsTiebreakDetails && tiebreakDetailsOpen && <StandingsTiebreakDetails leaders={dashboard.standingsLeaders} division={selectedTeam?.divisions}/>} 
