...
export default function CaptainDesignPreviewView({ dashboard = {} }) {
  ...
  const nextScheduleItem = dashboard.nextScheduleItem || null;
  ...
  const hasStandingsTiebreakDetails = hasStandingsTiebreak(leaders, selectedTeam?.divisions);
  ...
  function renderMatchSetupStatus(match, dialog = false, hero = false) {
    if (!match) return null;
    const matchSetupTeams = setupTeamsFor(match);
    const accessibleSetupTeams = captainSetupTeamsFor(match);
    const className = [captainStyles.matchSetupPanel, dialog ? captainStyles.dialogMatchSetupPanel : "", hero ? captainStyles.heroMatchSetupPanel : ""].filter(Boolean).join(" ");
    return (
      <section className={className}>
        <div className={captainStyles.setupHeading}><span>Match Setup</span><strong>Team lineups &amp; readiness</strong></div>
        <div className={captainStyles.setupStatuses}>{matchSetupTeams.map((setupTeam) => { const complete = setupStatusFor(match, setupTeam.id)?.complete === true; return <span className={complete ? captainStyles.setupComplete : captainStyles.setupPending} key={setupTeam.id}><span className={captainStyles.setupTeamName}>{setupTeam.side} - <b>{setupTeam.name}</b></span><span className={captainStyles.setupStatusDivider} aria-hidden="true">—</span><strong>{complete ? "Setup Complete" : "Setup Pending"}</strong></span>; })}</div>
  ...
  function renderMatchToolActions(match) {
    ...
    return (
      <div className={captainStyles.matchToolsActions}>
        ...
        <button type="button" className={captainStyles.blueAction} disabled={dashboard.readOnly} onClick={() => { setMatchToolsMatch(null); dashboard.onEmailOpposingCaptains?.(match); }}>Email Opposing Captains</button>
        ...
        {flexScheduleAvailable && <button type="button" className={captainStyles.blueAction} disabled={dashboard.readOnly || !flexScheduleAllowed} title={flexScheduleAllowed ? "Modify this Flex League match date/time" : "Only the home captain or co-captain can modify this match"} onClick={() => { if (!flexScheduleAllowed) return; setMatchToolsMatch(null); dashboard.onOpenFlexSchedule?.(match); }}>Modify Match Date/Time</button>}
        <button type="button" className={scoreEntryToneFor(match)} disabled={dashboard.readOnly || !canEnterMatchScores} onClick={() => { if (!canEnterMatchScores) return; setMatchToolsMatch(null); dashboard.onEnterMatchScores?.(match); }}>Enter Match Scores</button>
      </div>
    );
  }
  ...
  return (
    <main className={["full-screen-main", styles.page, captainStyles.captainPage, appearance.isLightCardHeaders ? styles.lightCardHeaders : "", appearance.isSidebarCollapsed ? styles.sidebarCollapsedLayout : ""].filter(Boolean).join(" ")} id="captain-preview-dashboard">
      ...
      <section className={styles.content}>
        <header className={styles.desktopHeader}><div><span>Captain dashboard</span><h1>Welcome {member?.first_name || displayName(member).split(" ")[0]}</h1></div><div className={styles.headerActions}><AskLwrTrigger role={role}/><a className={styles.helpButton} href={`mailto:${dashboard.contactEmail || "info@lwrpickleballclub.com"}`} aria-label="Email League Management"><Icon name="mail"/></a><div className={styles.userIdentity}><strong>{displayName(member)}</strong><small>{roleLabel(role)}</small></div><button type="button" className={styles.profileButton} onClick={() => setProfileOpen(true)}><Avatar person={member}/></button></div></header>
        <header className={styles.mobileHeader}><div className={styles.mobileLeading}><button type="button" className={styles.mobileMenuButton} onClick={() => setMobileMenuOpen(true)} aria-label="Open navigation menu" aria-expanded={mobileMenuOpen} aria-controls="captain-mobile-navigation"><Icon name="menu" size={19}/></button><a className={styles.mobileLogo} href="https://lwrpickleballclub.com" target="_blank" rel="noreferrer"><Image src="/lms-icon-192.png" width={36} height={36} alt=""/></a></div><div className={styles.mobileTitle}><strong>Captain Dashboard</strong><span>{teamName}</span></div><div className={styles.mobileActions}><AskLwrTrigger role={role} compact/><button type="button" onClick={() => setProfileOpen(true)} aria-label="Open profile"><Avatar person={member}/></button></div></header>
        ...
        <section className={`${styles.hero} ${captainStyles.hero}`}>
          ...
          <div className={styles.heroCopy}><div className={`${styles.nextMatch} ${styles.nextMatchHeader}`}><span>Next Match</span><div>{nextMatch && <span className={styles.matchType}>{selectedIsHome ? "Home Match" : "Away Match"}</span>}<b>{nextMatch ? `${nextDate.label} - ${shortTime(nextMatch.scheduled_time)}` : nextBye ? `${nextDate.label} - Bye Week` : "No upcoming match scheduled"}</b></div></div><h2>{nextMatch ? <>{teamName} <em>vs</em> {opponentFor(nextMatch, teamId)}</> : nextBye ? `${teamName} Bye Week` : "End of Regular Season - No More Matches"}</h2><div className={styles.location}><Icon name="pin" size={16}/><span>{nextMatch?.locations?.name || (nextBye ? "No match scheduled" : "No more match dates")}</span>{nextMatch?.week_number ? <><i>-</i><span>Week {nextMatch.week_number}</span></> : nextBye?.week_number ? <><i>-</i><span>Week {nextBye.week_number}</span></> : null}</div><div className={captainStyles.heroActions}><button type="button" className={captainStyles.blueAction} disabled={!nextMatch} onClick={() => nextMatch && dashboard.onOpenMatch?.(nextMatch)}>Match Details</button><button type="button" className={captainStyles.blueAction} disabled={dashboard.readOnly || !nextMatch} onClick={() => nextMatch && dashboard.onEmailOpposingCaptains?.(nextMatch)}>Email Opposing Captains</button><button type="button" className={bothTeamsSetupComplete ? captainStyles.successAction : captainStyles.dangerAction} disabled={!nextMatch} onClick={() => nextMatch && dashboard.onOpenMatchScoreSheet?.(nextMatch)}>Match Score Sheet</button>{nextMatch && flexScheduleAvailableFor(nextMatch) && <button type="button" className={captainStyles.blueAction} disabled={dashboard.readOnly || !flexScheduleAllowedFor(nextMatch)} title={flexScheduleAllowedFor(nextMatch) ? "Modify this Flex League match date/time" : "Only the home captain or co-captain can modify this match"} onClick={() => flexScheduleAllowedFor(nextMatch) && dashboard.onOpenFlexSchedule?.(nextMatch)}>Modify Match Date/Time</button>}<button type="button" className={scoreEntryTone} disabled={dashboard.readOnly || !canEnterScores} onClick={() => nextMatch && canEnterScores && dashboard.onEnterMatchScores?.(nextMatch)}>Enter Match Scores</button></div>{nextMatch && renderMatchSetupStatus(nextMatch, false, true)}</div>
        </section>
        ...
        <div className={captainStyles.dashboardGrid}>
          ...
          <section className={`${styles.card} ${captainStyles.pendingCard} ${pendingScoreActions.length ? captainStyles.pendingAlert : ""}`} id="captain-preview-pending"><Heading eyebrow="Action needed" title="Pending Match Score Entry/Validations"/><div className={captainStyles.matchList}>{pendingScoreActions.length ? pendingScoreActions.slice(0, 4).map(({ match, action }) => { const date = shortDate(match.scheduled_date); const needsEntry = action === "entry"; return <button type="button" className={captainStyles.matchRow} key={match.id} disabled={dashboard.readOnly && needsEntry} onClick={() => needsEntry ? dashboard.onEnterMatchScores?.(match) : dashboard.onOpenScoreDetails?.(match)} aria-label={`${needsEntry ? "Enter scores for" : "Review scores for"} ${match.home_team?.name || "Home"} versus ${match.away_team?.name || "Away"}`}><time><b>{date.day}</b><span>{date.month}</span></time><p><strong>{match.home_team?.name || "Home"} vs {match.away_team?.name || "Away"}</strong><span>{date.label} - {shortTime(match.scheduled_time)}</span></p><small>{needsEntry ? "Enter Scores" : "Validate"}</small><Icon name="arrow" size={16}/></button>; }) : <p className={captainStyles.empty}>No matches currently need score entry or validation.</p>}</div></section>

          <section className={[styles.card, captainStyles.matchesCard].join(" ")} id="captain-preview-matches"><Heading eyebrow="Plan for Upcoming Matches" title="Upcoming matches & byes" hint="Click Match for Match Tools" meta={{ label: "Matches", value: upcomingMatchCount }} action={upcomingItems.length > 4 ? (showAllUpcoming ? "Show Less" : "Show More") : ""} onAction={() => setShowAllUpcoming((show) => !show)}/><div className={captainStyles.matchList}>{visibleUpcomingItems.length ? visibleUpcomingItems.map((item, index) => { if (item.type === "bye") { const date = shortDate(item.data?.bye_date); return <div className={captainStyles.matchRow} key={"bye-" + (item.data?.id || index)}><time><b>{date.day}</b><span>{date.month}</span></time><p><strong>{teamName} Bye Week</strong><span>{date.label} - Week {item.data?.week_number || "-"}</span></p><small>Bye</small></div>; } const match = item.data; const date = shortDate(match.scheduled_date); const scoresNotVerified = match.score_status === "pending_verification"; const scoresNotEntered = Boolean(match.scheduled_date && match.scheduled_date < localDateString() && match.score_status === "not_entered" && !match.score_entered_at); const hasScoreAlert = scoresNotVerified || scoresNotEntered; const scoreAlertLabel = scoresNotEntered ? "Scores Not Entered" : "Scores Not Verified"; return <button type="button" className={[captainStyles.matchRow, hasScoreAlert ? captainStyles.overdueScoreRow : ""].filter(Boolean).join(" ")} key={match.id} onClick={() => setMatchToolsMatch(match)} aria-label={`Open Match Tools for ${match.home_team?.name || "Home"} versus ${match.away_team?.name || "Away"}`}><time><b>{date.day}</b><span>{date.month}</span></time><p><strong>{match.home_team?.name || "Home"} vs {match.away_team?.name || "Away"}</strong><span>{date.label} - {shortTime(match.scheduled_time)}</span></p><small>{hasScoreAlert ? scoreAlertLabel : String(match.home_team_id) === String(teamId) ? "Home" : "Away"}</small><Icon name="arrow" size={16}/></button>; }) : <p className={captainStyles.empty}>No upcoming matches or byes.</p>}</div></section>
