...
function Heading({ eyebrow, title, action, onAction, meta }) {
  return <div className={styles.heading}><div><span>{eyebrow}</span><h3>{title}</h3></div><div className={styles.headingAside}>{meta && <span className={styles.headingMeta}><small>{meta.label}</small><strong>{meta.value}</strong></span>}{action && <button type="button" onClick={onAction}>{action}<Icon name="arrow" size={15}/></button>}</div></div>;
}
...
export default function CaptainDesignPreviewView({ dashboard = {} }) {
  ...
  const pendingMatches = dashboard.pendingVerification || [];
  ...
  const navButton = (section, label, icon, detail = "", onClick) => <button type="button" className={activeSection === section ? styles.active : ""} onClick={onClick || (() => goToSection(section))}><Icon name={icon}/><span><b>{label}</b>{detail && <small>{detail}</small>}</span></button>;

  async function confirmLogout() {
    setLogoutPending(true);
    try { await dashboard.onLogout?.(); } finally { setLogoutPending(false); }
  }
  ...
  function renderMatchToolActions(match) {
    ...
    return (
      <div className={captainStyles.matchToolsActions}>
        <button type="button" className={styles.primary} onClick={() => { setMatchToolsMatch(null); dashboard.onOpenMatch?.(match); }}>View Match</button>
        <button type="button" className={captainStyles.blueAction} onClick={() => { setMatchToolsMatch(null); dashboard.onEmailOpposingCaptains?.(match); }}>Email Captains</button>
  ...
  return (
    <main className={`full-screen-main ${styles.page} ${captainStyles.captainPage}`} id="captain-preview-dashboard">
      <aside className={styles.sidebar}>
        ...
        <nav className={styles.sideNav} aria-label="Captain dashboard navigation">
          <div className={styles.navGroup}>
            {navButton("dashboard", "Dashboard", "dashboard", "Captain", () => setExpandedMenu((current) => current === "dashboard" ? "" : "dashboard"))}
            {expandedMenu === "dashboard" && <div className={styles.submenu}><button type="button" onClick={() => dashboard.onChangeDashboard?.("/design-preview")}><span>Player Dashboard</span></button><button type="button" className={styles.submenuActive} aria-current="page" onClick={() => goToSection("dashboard")}><span>Captain Dashboard</span></button>{canUseAdminDashboard && <button type="button" onClick={() => dashboard.onChangeDashboard?.("/design-preview/admin")}><span>Admin Dashboard</span></button>}</div>}
          </div>
          <div className={styles.navGroup}>
            {navButton("team", "My Team", "team", teams.length > 1 ? teamName : "", () => teams.length > 1 ? setExpandedMenu((current) => current === "team" ? "" : "team") : goToSection("team"))}
          ...
          <div className={styles.navGroup}>{navButton("documents", "League Documents", "document", leagueName, () => setExpandedMenu((current) => current === "documents" ? "" : "documents"))}{expandedMenu === "documents" && <div className={styles.submenu}>{documents.length ? documents.map((document) => <button type="button" key={document.key} onClick={() => dashboard.onOpenLeagueDocument?.(document)}><span>{document.label}</span></button>) : <p>No documents are published for this league.</p>}</div>}</div>
        </nav>
      ...
      <section className={styles.content}>
        ...
        <label className={`${styles.mobileDashboardSelect} ${captainStyles.mobileDashboardSelect}`}><span>Dashboard view</span><select value="/design-preview/captain" onChange={(event) => dashboard.onChangeDashboard?.(event.target.value)}><option value="/design-preview">Player Dashboard</option><option value="/design-preview/captain">Captain Dashboard</option>{canUseAdminDashboard && <option value="/design-preview/admin">Admin Dashboard</option>}</select></label>
        ...
        <section className={styles.stats} aria-label="Captain team statistics">
          ...
          <button type="button" className={styles.statCard} onClick={() => openNavigationSection("standings")} aria-label="Open Division Standings"><div><Icon name="trophy"/></div><span>Division rank<strong>{standing?.rank ? `#${standing.rank}` : "--"}</strong><small>{leagueName} <b>|</b> {divisionName}</small></span></button>
          <article className={pendingMatches.length ? captainStyles.pendingStat : ""}><div><Icon name="verify"/></div><span>Pending match verifications<strong>{pendingMatches.length}</strong><small>{pendingMatches.length ? "Captain review needed" : "Nothing awaiting review"}</small></span></article>
        </section>

        <div className={captainStyles.dashboardGrid}>
          <section className={`${styles.card} ${captainStyles.actionsCard}`} id="captain-preview-actions"><Heading eyebrow="Captain workspace" title={teamName}/><div className={captainStyles.actionGrid}><button type="button" onClick={() => dashboard.onOpenRoster?.()}><Icon name="team"/><span><strong>View Team</strong><small>{stats.playerCount || 0} rostered players</small></span></button><button type="button" onClick={() => dashboard.onManageRoster?.()}><Icon name="team"/><span><strong>Manage Roster</strong><small>Players and leadership</small></span></button><button type="button" onClick={() => openNavigationSection("standings")}><Icon name="trophy"/><span><strong>Standings</strong><small>Full division view</small></span></button><button type="button" onClick={() => dashboard.onOpenCaptains?.()}><Icon name="mail"/><span><strong>Division Captains</strong><small>Captain contact list</small></span></button></div></section>

          <section className={`${styles.card} ${captainStyles.pendingCard} ${pendingMatches.length ? captainStyles.pendingAlert : ""}`} id="captain-preview-pending"><Heading eyebrow="Action needed" title="Pending Match Verifications"/><div className={captainStyles.compactList}>{pendingMatches.length ? pendingMatches.slice(0, 4).map((match) => <button type="button" key={match.id} onClick={() => dashboard.onOpenScoreDetails?.(match)}><span><strong>{match.home_team?.name || "Home"} vs {match.away_team?.name || "Away"}</strong><small>{shortDate(match.scheduled_date).label}</small></span><b>Review</b></button>) : <p className={captainStyles.empty}>No matches currently need verification.</p>}</div></section>

          <section className={[styles.card, captainStyles.matchesCard].join(" ")} id="captain-preview-matches"><Heading eyebrow="Plan for Upcoming Matches" title="Upcoming matches & byes" meta={{ label: "Matches", value: upcomingMatchCount }}/><div className={captainStyles.matchList}>{upcomingItems.length ? upcomingItems.slice(0, 4).map((item, index) => { if (item.type === "bye") { const date = shortDate(item.data?.bye_date); return <div className={captainStyles.matchRow} key={"bye-" + (item.data?.id || index)}><time><b>{date.day}</b><span>{date.month}</span></time><p><strong>{teamName} Bye Week</strong><span>{date.label} - Week {item.data?.week_number || "-"}</span></p><small>Bye</small></div>; } const match = item.data; const date = shortDate(match.scheduled_date); return <div className={captainStyles.matchRow} key={match.id}><time><b>{date.day}</b><span>{date.month}</span></time><button type="button" className={captainStyles.matchSummaryButton} onClick={() => dashboard.onOpenMatch?.(match)}><strong>{match.home_team?.name || "Home"} vs {match.away_team?.name || "Away"}</strong><span>{date.label} - {shortTime(match.scheduled_time)}</span></button><div className={captainStyles.matchRowActions}><small>{String(match.home_team_id) === String(teamId) ? "Home" : "Away"}</small><button type="button" onClick={() => setMatchToolsMatch(match)}>Match Tools</button></div></div>; }) : <p className={captainStyles.empty}>No upcoming matches or byes.</p>}</div></section>
          ...
          <section className={[styles.card, captainStyles.resultsCard, styles.resultsCard].join(" ")} id="captain-preview-results"><Heading eyebrow="Completed" title="Recent results" meta={{ label: "Total Team Points", value: Number(standing?.standings_points || 0) }} action={orderedCompletedMatches.length > 3 ? (showAllResults ? "Show Less" : "Show More") : ""} onAction={() => setShowAllResults((show) => !show)}/><div className={styles.results}>{visibleCompletedMatches.length ? visibleCompletedMatches.map((match) => { const isHome = String(match.home_team_id) === String(teamId); const selectedScore = isHome ? match.home_score : match.away_score; const opponentScore = isHome ? match.away_score : match.home_score; const won = String(match.winning_team_id || "") === String(teamId); const tied = !match.winning_team_id && Number(selectedScore) === Number(opponentScore); const mark = tied ? "T" : won ? "W" : "L"; const tone = tied ? "tie" : won ? "win" : "loss"; const rowToneClass = tied ? styles.tieRow : won ? styles.winRow : styles.lossRow; return <div className={[styles.resultRow, rowToneClass].join(" ")} key={match.id}><b className={styles[tone]}>{mark}</b><button type="button" className={styles.resultSummary} onClick={() => dashboard.onOpenScoreDetails?.(match)}><strong>{match.home_team?.name || "Home"} <span>vs</span> {match.away_team?.name || "Away"}</strong><small>{shortDate(match.scheduled_date).label} - {isHome ? "Home" : "Away"}</small></button><strong className={styles[tone + "Score"]}>{match.home_score ?? "-"}-{match.away_score ?? "-"}</strong><button type="button" className={styles.detailsButton} onClick={() => dashboard.onOpenScoreDetails?.(match)}>Match Score Details</button></div>; }) : <p className={captainStyles.empty}>No verified results are available.</p>}</div></section>
        </div>
        <p className={styles.note}>Live Captain preview - current data, permissions, and existing operational tools</p><p className={styles.mobileCopyright}>&copy; {currentYear} Lakewood Ranch Pickleball Club</p>
      </section>
      ...
      {logoutConfirmOpen && <div className={`${styles.modalLayer} ${styles.confirmLayer}`} role="alertdialog" aria-modal="true"><button type="button" className={styles.backdrop} onClick={() => !logoutPending && setLogoutConfirmOpen(false)} aria-label="Cancel logout"/><section className={styles.confirmDialog}><span>Account</span><h2>Log out of this device?</h2><p>You will remain signed in on your other browsers and devices.</p><div><button type="button" onClick={() => setLogoutConfirmOpen(false)} disabled={logoutPending}>Stay signed in</button><button type="button" className={styles.confirmLogout} onClick={confirmLogout} disabled={logoutPending}>{logoutPending ? "Logging out..." : "Log out"}</button></div></section></div>}
    </main>
  );
}
