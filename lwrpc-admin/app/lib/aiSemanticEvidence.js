...
export async function assessSemanticEvidence({question,candidates,fetchImpl=fetch,repairReason=null}) {
  ...
  const response=await fetchImpl('https://api.openai.com/v1/responses',{
    ...
    body:JSON.stringify({model:aiAssistantConfig.chatModel,store:false,reasoning:{effort:'low'},max_output_tokens:800,
      text:{format:{type:'json_schema',name:'official_evidence_relevance',strict:true,schema:{type:'object',additionalProperties:false,required:['supported','chunkIds','reason'],properties:{supported:{type:'boolean'},chunkIds:{type:'array',maxItems:4,items:{type:'string',enum:candidates.map(c=>c.chunkId)}},reason:{type:'string'}}}}},
      instructions:[
      ...
      ].join('\n'),input:JSON.stringify({question,candidates:candidates.map(c=>({chunkId:c.chunkId,title:c.documentTitle,type:c.documentType,authority:c.documentAuthorityRank,heading:c.heading,content:c.content}))})}),
  });
...
export async function selectSemanticEvidence(retrieval,{assess=assessSemanticEvidence,qualifies}) {
  let characters=0;
  const candidates=retrieval.candidates.filter(c=>qualifies(c)&&leagueCompatible(c,retrieval.request.question)).slice(0,12).filter(c=>{
  ...
  for(let attempt=0;attempt<2;attempt++){
    assessment=await assess({question:retrieval.request.question,candidates,repairReason:diagnostic.attempts.at(-1)?.rejection||null});
  ...
  const relevant=candidates.filter(c=>ids.includes(c.chunkId));
