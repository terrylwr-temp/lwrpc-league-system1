            ...
            <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
              <table className="w-full min-w-[1120px] border-collapse text-sm">
                ...
                <tbody>{standings.map((row, index) => {
                  ...
                  return <tr key={row.id || row.team_id} className="border-b border-slate-100 hover:bg-slate-50"><td className="p-3 font-black">#{index + 1}</td><td className="p-3 font-bold text-slate-950">{row.teams?.name || "Team"}</td><td className="p-3 text-center font-black text-emerald-700">{points}</td><td className="p-3 text-center">{played}</td><td className="p-3 text-center">{row.match_wins || 0}</td><td className="p-3 text-center">{row.match_losses || 0}</td><td className="p-3 text-center">{row.match_ties || 0}</td><td className="p-3 text-center">{row.game_wins || 0}-{row.game_losses || 0}</td><td className="p-3 text-center">{played ? (points / played).toFixed(2) : "0.00"}</td><td className="p-3 text-center font-mono font-bold">{row.recent_form || "-"}</td><td className="p-3 text-center">{row.points_for || 0}</td><td className="p-3 text-center">{row.points_against || 0}</td><td className="p-3 text-center font-bold">{row.point_differential || 0}</td></tr>;
                })}</tbody>
              </table>
