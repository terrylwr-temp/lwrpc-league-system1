...
export default function RatingsImportPreview({preview,busy,onImport}) {
 const [page,setPage]=useState(0);const pages=Math.max(1,Math.ceil(preview.rows.length/50)),current=Math.min(page,pages-1);
 return <section className="mt-4 rounded-xl border border-blue-200 bg-white p-4" aria-label="Ratings import preview">
  ...
  <div className="overflow-x-auto" role="region" aria-label="CSV values and proposed fills" tabIndex={0}><table className="w-full text-left text-sm">
   ...
   <tbody>{preview.rows.slice(current*50,(current+1)*50).map((row,i)=><tr key={`${row.line}-${row.memberId||i}`} className="border-b border-slate-200">
  ...
  <nav aria-label="Import preview pages" className="mt-3 flex items-center gap-4"><button type="button" disabled={current===0} onClick={()=>setPage(current-1)}>Previous</button><span>Page {current+1} of {pages}</span><button type="button" disabled={current+1>=pages} onClick={()=>setPage(current+1)}>Next</button></nav>
 </section>;
}
