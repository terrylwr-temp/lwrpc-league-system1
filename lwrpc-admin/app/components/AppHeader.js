...
export default function AppHeader({
  ...
  async function saveHeaderProfileImage(file) {
  ...
  return (
    <>
      <aside className={[adminShellStyles.sidebar, appearance.isLightSidebar ? adminShellStyles.lightSidebar : "", appearance.isSidebarCollapsed ? adminShellStyles.sidebarCollapsed : "", appearance.isSidebarCollapsed ? "lwrpc-sidebar-collapsed" : ""].filter(Boolean).join(" ")} style={{ "--blue": "#1558d5" }} aria-label="League Management navigation; hover to expand when collapsed" onMouseLeave={() => { if (appearance.isSidebarCollapsed) setOpenGroup(""); }}>
      ...
      <header className="sticky top-0 z-30 -mx-4 -mt-4 mb-3 grid min-h-[72px] grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 border-b border-[#dce4ef] bg-[#f7f9fc]/95 px-4 py-[env(safe-area-inset-top)] shadow-sm backdrop-blur-xl min-[701px]:hidden">
        ...
        <div className="flex items-center gap-1"><button type="button" onClick={() => setAssistantOpen(true)} className="grid h-9 w-9 place-items-center rounded-full border border-[#dce4ef] bg-white text-[#536079]" aria-label="Ask LWR Pickleball AI" title="Ask LWR Pickleball AI"><Icon name="sparkles" size={18}/></button><button type="button" onClick={() => setProfileOpen(true)} className="grid h-10 w-10 place-items-center rounded-full bg-transparent" aria-label="Open profile"><ProfileAvatar member={member} size={36}/></button></div>
      </header>
      ...
      {menuOpen && <div className="fixed inset-0 z-[60] min-[701px]:hidden"><button type="button" className="absolute inset-0 bg-slate-950/60" onClick={() => setMenuOpen(false)} aria-label="Close navigation"/><aside className={["relative flex h-full w-[min(88vw,320px)] flex-col overflow-y-auto p-5 shadow-2xl", appearance.isLightSidebar ? adminShellStyles.lightSidebar : "bg-gradient-to-b from-[#102e64] to-[#0d2249] text-white"].filter(Boolean).join(" ")} style={{ "--blue": "#1558d5" }}><div className="flex items-center justify-between gap-3"><a href={clubWebsite} target="_blank" rel="noreferrer" className="flex min-w-0 items-center gap-3"><Image src={logoUrl} alt={clubName} width={46} height={46} className="h-11 w-11 rounded-full bg-white object-contain" unoptimized/><strong className={`text-sm font-black leading-tight ${appearance.isLightSidebar ? "text-[#17345b]" : "text-white"}`}>{clubName}</strong></a><button type="button" onClick={() => setMenuOpen(false)} className={`grid h-10 w-10 shrink-0 place-items-center rounded-full text-xl ${appearance.isLightSidebar ? "bg-[#e3ebf4] text-[#17345b]" : "bg-white/10 text-white"}`} aria-label="Close navigation">{"\u00D7"}</button></div><Navigation className="mt-5"/>{mobileSidebarAction && <div className="mt-5" onClick={() => setMenuOpen(false)}>{mobileSidebarAction}</div>}<p className={`mt-auto pt-6 text-center text-[11px] font-bold ${appearance.isLightSidebar ? "text-[#6e7f93]" : "text-[#afc1dd]"}`}>{"\u00A9"} {new Date().getFullYear()} {clubName}</p></aside></div>}

      {false && profileOpen && <div className="fixed inset-0 z-[70] grid place-items-center p-4" role="dialog" aria-modal="true" aria-labelledby="shared-profile-title"><button type="button" className="absolute inset-0 bg-slate-950/65 backdrop-blur-sm" onClick={() => setProfileOpen(false)} aria-label="Close profile"/><section className="relative w-full max-w-[500px] overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-2xl"><header className="flex items-center gap-4 bg-gradient-to-r from-[#102e64] to-[#1558d5] px-5 py-5 text-white"><ProfileAvatar member={member} size={54}/><div className="min-w-0 flex-1"><span className="text-[11px] font-black uppercase tracking-[.12em] text-[#bcd1f6]">Signed-in profile</span><h2 id="shared-profile-title" className="truncate text-[23px] font-black">{displayName}</h2><p className="truncate text-[13px] font-semibold text-[#d8e5fb]">{roleLabel(role)}{member?.email ? ` - ${member.email}` : ""}</p></div><button type="button" onClick={() => setProfileOpen(false)} className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-white/25 bg-white/10 text-2xl" aria-label="Close profile">{"\u00D7"}</button></header><div className="grid gap-2 p-4">
