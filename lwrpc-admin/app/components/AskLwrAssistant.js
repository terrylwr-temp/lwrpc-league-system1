...
import { ASK_LWR_INITIAL_COPY, assistantPageContext } from "../lib/askLwrAssistantConfig";
...
export function AskLwrAssistantDrawer({ open, onClose, role }) {
  ...
  return <div className="fixed inset-0 z-[90]" role="dialog" aria-modal="true" aria-labelledby="ask-lwr-title">
    <button type="button" className="absolute inset-0 bg-slate-950/45 backdrop-blur-[1px]" onClick={onClose} aria-label="Close Ask LWR Pickleball AI"/>
...
function AssistantContent({ role = "player", inputRef, closeButtonRef, onClose, drawer = false }) {
  ...
  async function submit(event, suggestedQuestion = "") {
    ...
    setQuestion(""); setError(""); setWorking(true);
    setExchanges((current) => [...current, { id: exchangeId, question: nextQuestion, pending: true }]);
    try {
  ...
  const guides = GUIDE_DOCUMENT_TYPES.filter((guide) => guide.key === "player_guide_pdf" || (guide.key === "captain_guide_pdf" && hasRole(role, "captain")) || (guide.key === "admin_guide_pdf" && hasRole(role, "league_manager")));
  ...
  return <>
    <header className="flex items-start gap-3 border-b border-slate-200 bg-white px-4 py-4 sm:px-6">
      ...
      <div className="min-w-0 flex-1"><h2 id="ask-lwr-title" className="text-lg font-black text-[#102e64]">Ask LWR Pickleball AI</h2><p id="ask-lwr-subtitle" className="mt-0.5 text-sm font-semibold leading-5 text-slate-600">Get answers from official LWR Pickleball Club rules and guides.</p></div>
      {onClose && <button ref={closeButtonRef} type="button" onClick={onClose} className="grid h-9 w-9 shrink-0 place-items-center rounded-full text-xl text-slate-600 transition hover:bg-slate-100 hover:text-slate-950" aria-label="Close Ask LWR Pickleball AI">×</button>}
    </header>
    <div className={bodyClass}>
      <div className="space-y-4">
        {exchanges.length === 0 && <section className="rounded-2xl border border-blue-100 bg-white p-4 shadow-sm"><h3 className="text-base font-black text-[#102e64]">How can I help?</h3><p className="mt-2 text-sm font-semibold leading-6 text-slate-600">{ASK_LWR_INITIAL_COPY}</p><div className="mt-4 flex flex-wrap gap-2">{pageContext.suggestions.map((suggestion) => <button key={suggestion} type="button" onClick={() => submit(null, suggestion)} disabled={working} className="rounded-full border border-blue-200 bg-blue-50 px-3 py-2 text-left text-xs font-bold leading-4 text-blue-800 transition hover:border-blue-400 hover:bg-blue-100 disabled:cursor-wait disabled:opacity-60">{suggestion}</button>)}</div></section>}
        {exchanges.map((entry) => <Exchange key={entry.id} entry={entry}/>) }
        {working && <p className="flex items-center gap-2 rounded-xl border border-blue-100 bg-blue-50 px-3 py-3 text-sm font-bold text-blue-900" role="status" aria-live="polite"><span className="h-3 w-3 animate-pulse rounded-full bg-blue-600" aria-hidden="true"/>Checking official LWR rules and guides...</p>}
        {error && <p className="rounded-xl border border-red-200 bg-red-50 px-3 py-3 text-sm font-bold text-red-800" role="alert">{error}</p>}
      </div>
      <div className="mt-5 border-t border-slate-200 pt-4">
        <form onSubmit={submit} className="flex gap-2"><label className="sr-only" htmlFor="ask-lwr-question">Ask a question about LWR Pickleball Club</label><textarea id="ask-lwr-question" ref={inputRef} value={question} maxLength={1000} rows={3} disabled={working} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); submit(event); } }} placeholder="Ask a question about LWR Pickleball Club..." className="min-h-[74px] flex-1 resize-y rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm font-semibold leading-5 text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 disabled:bg-slate-100"/><button type="submit" disabled={working || !question.trim()} className="self-end rounded-xl bg-[#1558d5] px-4 py-2.5 text-sm font-black text-white shadow-sm transition hover:bg-[#104ab7] disabled:cursor-not-allowed disabled:bg-slate-300">Ask</button></form>
        {exchanges.length > 0 && <button type="button" onClick={() => { setExchanges([]); setError(""); setQuestion(""); }} disabled={working} className="mt-3 text-sm font-bold text-blue-700 hover:text-blue-900 disabled:text-slate-400">New Conversation</button>}
        <div className="mt-4 rounded-xl border border-slate-200 bg-white"><button type="button" onClick={toggleGuides} aria-expanded={guidesOpen} className="flex w-full items-center justify-between gap-3 px-3 py-3 text-left text-sm font-black text-[#102e64]"><span>Browse Guides &amp; Rules</span><span aria-hidden="true">{guidesOpen ? "−" : "+"}</span></button>{guidesOpen && <div className="border-t border-slate-200 p-3"><p className="mb-3 text-xs font-semibold leading-5 text-slate-600">Open the official user guides and league documents already available in the LMS.</p><div className="grid gap-2">{guides.map((guide) => <button key={guide.key} type="button" onClick={() => openGuideDocument(supabase, guide)} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-left text-sm font-bold text-blue-800 hover:border-blue-300 hover:bg-blue-50">{guide.label}</button>)}{leagueGuides.map((guide) => <button key={guide.key} type="button" onClick={() => openLeagueGuide(guide)} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-left text-sm font-bold text-blue-800 hover:border-blue-300 hover:bg-blue-50">{guide.label}</button>)}{guidesLoading && <p className="text-sm font-semibold text-slate-500" role="status">Loading league documents...</p>}{!guidesLoading && guides.length + leagueGuides.length === 0 && <p className="text-sm font-semibold text-slate-500">No user-facing guides are configured yet.</p>}</div></div>}</div>
      </div>
    </div>
  </>;
}
...
function Exchange({ entry }) {
  if (entry.pending) return <article className="rounded-2xl border border-slate-200 bg-white p-4"><p className="text-sm font-bold text-slate-700">{entry.question}</p></article>;
  if (entry.requestError) return <article className="rounded-2xl border border-red-200 bg-white p-4"><p className="text-sm font-bold text-slate-700">{entry.question}</p><p className="mt-3 text-sm font-semibold text-red-800">{TECHNICAL_ERROR}</p></article>;
  const result = entry.result;
  return <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700">{entry.question}</div><div className="p-4"><h3 className="text-sm font-black uppercase tracking-[.1em] text-[#102e64]">Answer</h3><p className="mt-2 whitespace-pre-wrap text-sm font-semibold leading-6 text-slate-700">{result.answer}</p>{result.sources?.length > 0 && <div className="mt-4 border-t border-slate-100 pt-4"><h3 className="text-sm font-black uppercase tracking-[.1em] text-[#102e64]">Official Source{result.sources.length > 1 ? "s" : ""}</h3><div className="mt-2 grid gap-2">{result.sources.map((source, index) => <a key={`${source.officialDocumentUrl}-${index}`} href={source.officialDocumentUrl} target="_blank" rel="noreferrer" className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900 transition hover:border-blue-300 hover:bg-blue-100"><strong className="block">{source.documentTitle}</strong><span className="mt-0.5 block font-semibold">{source.citation}</span><span className="mt-2 inline-block font-black text-blue-700">View Official Document ↗</span></a>)}</div></div>}</div></article>;
}
...
async function loadLeagueGuides(role) {
  ...
  return (data || []).flatMap((row) => {
    ...
    return LEAGUE_DOCUMENT_TYPES.filter((type) => type.key !== "captains_guide" || hasRole(role, "captain")).map((type) => {
