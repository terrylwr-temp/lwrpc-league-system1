...
function AssistantContent({ role = "player", inputRef, closeButtonRef, onClose, drawer = false }) {
  ...
  return <>
    <header className="flex items-start gap-3 border-b border-slate-200 bg-white px-4 py-4 sm:px-6">
      ...
      <div className="min-w-0 flex-1"><h2 id="ask-lwr-title" className="text-lg font-black text-[#102e64]">Ask LWR Pickleball Club AI</h2><p id="ask-lwr-subtitle" className="mt-0.5 text-sm font-semibold leading-5 text-slate-600">Get answers from official LWR Pickleball Club rules and guides.</p></div>
...
function Exchange({ entry }) {
  if (entry.pending) return <article className="rounded-2xl border border-slate-200 bg-white p-4"><h3 className="text-sm font-black uppercase tracking-[.1em] text-[#102e64]">Question</h3><p className="mt-2 text-sm font-bold text-slate-700">{entry.question}</p><p className="mt-3 flex items-center gap-2 text-sm font-bold text-blue-900" role="status" aria-live="polite"><span className="h-3 w-3 animate-pulse rounded-full bg-blue-600" aria-hidden="true"/>Finding the official answer...</p></article>;
  if (entry.requestError) return <article className="rounded-2xl border border-red-200 bg-white p-4"><h3 className="text-sm font-black uppercase tracking-[.1em] text-[#102e64]">Question</h3><p className="mt-2 text-sm font-bold text-slate-700">{entry.question}</p><p className="mt-3 text-sm font-semibold text-red-800">{TECHNICAL_ERROR}</p></article>;
  const result = entry.result;
  return <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 bg-slate-50 px-4 py-3"><h3 className="text-sm font-black uppercase tracking-[.1em] text-[#102e64]">Question</h3><p className="mt-2 text-sm font-bold text-slate-700">{entry.question}</p></div><div className="p-4"><h3 className="text-sm font-black uppercase tracking-[.1em] text-[#102e64]">Answer</h3><p className="mt-2 whitespace-pre-wrap text-sm font-semibold leading-6 text-slate-700">{result.answer}</p>{result.sources?.length > 0 && <div className="mt-4 border-t border-slate-100 pt-4"><h3 className="text-sm font-black uppercase tracking-[.1em] text-[#102e64]">Official Source{result.sources.length > 1 ? "s" : ""}</h3><div className="mt-2 grid gap-2">{result.sources.map((source, index) => <a key={`${source.officialDocumentUrl}-${index}`} href={source.officialDocumentUrl} target="_blank" rel="noreferrer" className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900 transition hover:border-blue-300 hover:bg-blue-100"><strong className="block">{source.documentTitle}</strong><span className="mt-0.5 block font-semibold">{source.citation}</span><span className="mt-2 inline-block font-black text-blue-700">View Official Document ↗</span></a>)}</div></div>}</div></article>;
}
