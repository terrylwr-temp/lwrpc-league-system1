...
test('0716 unvoted grounded metadata only; correlated receipt and unchanged result keys',async()=>{
 ...
 assert.deepEqual(Object.keys(e.result).sort(),['answer','conflict','conversationReceipt','evidenceSufficient','feedbackReceipt','kind','provenance','sources'].sort());
});
...
test('0716 build/oversize/write/logger/timeout failures fail open',async()=>{
 ...
 assert.equal(await persistQuality(hanging,()=>({}),{budgetMs:10,log:(...a)=>logs.push(a)}),false);assert.equal(aborted,true);assert.equal(logs.length,1);assert.deepEqual(logs[0].slice(0,2),['capture_failed','persistence']);assert.equal(logs[0][2].reason,'DEADLINE');
});
