-- LOCAL REVIEW ONLY. Separate input fill, clear and regular recalculation transactions.
begin;
alter table public.member_season_ratings add column if not exists dupr_age_based_rating numeric;
create schema if not exists ratings_workflow_private;
revoke all on schema ratings_workflow_private from public,anon,authenticated,service_role;
create table if not exists ratings_workflow_private.input_state(
 season_id uuid not null,member_id uuid not null,basis jsonb not null default '{}',cleared_at timestamptz,
 primary key(season_id,member_id));
create table if not exists ratings_workflow_private.runs(
 id uuid primary key,actor uuid not null,season_id uuid not null,operation text not null,
 payload jsonb not null,before_rows jsonb not null,result jsonb not null,created_at timestamptz not null default clock_timestamp());
alter table ratings_workflow_private.input_state enable row level security;
alter table ratings_workflow_private.runs enable row level security;
revoke all on all tables in schema ratings_workflow_private from public,anon,authenticated,service_role;

create or replace function ratings_workflow_private.policy() returns jsonb
language plpgsql security definer set search_path='' as $$
declare content text; version uuid; n int; threshold numeric; adjustment numeric;
begin
 select count(*),min(d.active_version_id::text)::uuid into n,version from public.ai_documents d
 join public.ai_document_versions v on v.id=d.active_version_id
 where d.status='active' and d.document_type='league_rules' and d.scope_kind='all' and v.processing_status='ready';
 if n<>1 then raise exception 'One authoritative active Rules version required';end if;
 select string_agg(c.content,E'\n' order by c.chunk_ordinal) into content from public.ai_document_chunks c where c.document_version_id=version and c.is_searchable;
 threshold:=substring(content from '4\.1\.1\.[\s\S]*?Reliability Factor of ([0-9]+(?:\.[0-9]+)?) or below')::numeric;
 adjustment:=substring(split_part(split_part(content,'4.5.1.',2),'4.5.2.',1) from 'minus ([0-9]+(?:[.][0-9]+)?)')::numeric;
 if threshold is null or threshold not between 0 and 100 or adjustment is null or adjustment<=0
 or content !~ '4\.2\. All DUPR ratings will be truncated to the nearest tenth'
 or content !~ '4\.5\.2\.[\s\S]*?highest adjusted Season' then raise exception 'Active Rules calculation requires review';end if;
 return jsonb_build_object('version',version,'hash',md5(content),'threshold',threshold,'adjustment',adjustment,'scale',1,'nrAdmission',content ~ 'Rated[)] are eligible to participate in any division[.]');
end $$;

create or replace function ratings_workflow_private.valid_source(s jsonb) returns boolean
...
create or replace function ratings_workflow_private.context(p_actor uuid,p_season uuid) returns jsonb
language plpgsql security definer set search_path='' as $$
...
create or replace function ratings_workflow_private.plan(p_actor uuid,p_season uuid,p_operation text,p_upload jsonb default '[]') returns jsonb
...
begin
  ...
  if row->>'memberId' is null then reason:='DUPR ID not found';
   ...
   if p_operation='transfer' and (s is null or s='null'::jsonb) then reason:='No imported source';
    ...
    foreach field in array array['doubles','rf','age'] loop
     ...
     else fills:=fills||jsonb_build_object(field,case when field='doubles' then to_jsonb(case when s->>'doubles'='NR' then 'NR' else trunc((s->>'doubles')::numeric,1)::text end) when field='age' then to_jsonb(trunc((s->>'age')::numeric,1)) else s->field end);end if;
    end loop;
 ...
 return jsonb_build_object('season',ctx->'season','rules',ctx->'rules','operation',p_operation,'fingerprint',md5(ctx::text||p_upload::text),
...
create or replace function ratings_workflow_private.commit_run(p_actor uuid,p_payload jsonb) returns jsonb
language plpgsql security definer set search_path='' set lock_timeout='1s' set statement_timeout='8s' as $$
...
begin
 ...
 begin
  ...
  plan:=ratings_workflow_private.plan(p_actor,season,operation,coalesce(p_payload->'upload','[]'));
  ...
  for row in select value from jsonb_array_elements(plan->'rows') where value->>'action' in ('FILL','RECORD','CLEAR','CREATE','UPDATE') loop
   ...
   if operation in ('upload','transfer') then
    ...
    update public.member_season_ratings set season_dupr_rating=(row->>'proposed')::numeric,notes=row->>'proposedNotes' where member_id=member and season_id=season;
  ...
  result:=jsonb_build_object('status','success','counts',plan->'counts','basis',jsonb_build_object('rules',plan->'rules','rows',plan->'rows'),'primeTimeUpdates',0);
...
create or replace function public.season_ratings_workflow_preview(p_actor uuid,p_season uuid,p_operation text,p_upload jsonb default '[]') returns jsonb
language sql security invoker set search_path='' as $$select ratings_workflow_private.plan(p_actor,p_season,p_operation,p_upload)$$;
create or replace function public.season_ratings_workflow_commit(p_actor uuid,p_payload jsonb) returns jsonb
language sql security invoker set search_path='' as $$select ratings_workflow_private.commit_run(p_actor,p_payload)$$;
create or replace function public.season_ratings_roster_policy() returns jsonb
language plpgsql security definer set search_path='' as $$
declare identity jsonb;
begin
 identity:=ai_live_private.resolve_identity(auth.uid());
 if coalesce(identity->>'role','') not in ('captain','league_manager','commissioner') then raise exception 'Roster authorization required';end if;
 return ratings_workflow_private.policy();
end $$;
revoke all on function public.season_ratings_roster_policy() from public,anon,authenticated,service_role;
grant execute on function public.season_ratings_roster_policy() to authenticated;
revoke all on all functions in schema ratings_workflow_private from public,anon,authenticated,service_role;
revoke all on function public.season_ratings_workflow_preview(uuid,uuid,text,jsonb),public.season_ratings_workflow_commit(uuid,jsonb) from public,anon,authenticated,service_role;
grant usage on schema ratings_workflow_private to service_role;
grant execute on function ratings_workflow_private.plan(uuid,uuid,text,jsonb),ratings_workflow_private.commit_run(uuid,jsonb),public.season_ratings_workflow_preview(uuid,uuid,text,jsonb),public.season_ratings_workflow_commit(uuid,jsonb) to service_role;
commit;
