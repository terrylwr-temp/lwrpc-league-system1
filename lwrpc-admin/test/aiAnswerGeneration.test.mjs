...
test("uses the explicit ownership FK in the production dual-relationship schema and revalidates the cited searchable chunk", async () => {
  ...
  const chunkQuery = { select(value) { calls.push(["chunks-select", value]); return this; }, in(column, values) { calls.push(["chunks-in", column, values]); return Promise.resolve({ data: [{ id: "chunk-1", document_version_id: "version-1", is_searchable: true, page_number: 5, rule_number: "5.7", section_label: "Incomplete Matches", heading: "Incomplete Matches", content: evidence[0].content }], error: null }); } };
  ...
  assert.deepEqual(calls[2], ["chunks-select", "id, document_version_id, is_searchable, page_number, rule_number, section_label, heading, content"]);
