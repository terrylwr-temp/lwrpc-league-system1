...
test('0727 Rule 3.5 policy bypasses Approved Answers; standalone conflict controls remain',async()=>{
 ...
 assert.equal(retrieval.authorityWarnings,undefined);
 assert.equal(managedCalls,0);assert.equal(models,1);
 ...
 assert.equal(outcome.final_kind,'answer');assert.equal(outcome.diagnostic_snapshot.authorityWarnings?.length||0,0);assert.equal(qualityException(outcome,execution).p_occurrence,null);
