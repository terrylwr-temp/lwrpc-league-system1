...
export async function replay(c,{select=true}={}){
 ...
 const db={from:table=>{if(table==='ai_documents'){const versions=new Set(chunks.map(x=>x.document_version_id));const rows=fixture.cases.flatMap(x=>(x.searches||[]).flatMap(s=>s.rows)).filter(x=>versions.has(x.document_version_id));return queryRows([...new Map(rows.map(r=>[r.document_id,{id:r.document_id,title:r.document_title,document_type:r.document_type,authority_rank:r.document_authority_rank,active_version_id:r.document_version_id,status:'active','active_version.processing_status':'ready',active_version:{id:r.document_version_id,processing_status:'ready'}}])).values()]);}if(table==='ai_document_chunks')return queryRows(chunks);throw new Error('Unexpected read '+table);},rpc:async(name,args)=>{if(name!=='search_ai_official_chunks')return {data:[]};const s=c.searches[calls++];if(!s)throw new Error('Unrecorded search');if(s.query!==args.p_query_text)throw new Error('Projection differs: '+args.p_query_text);return {data:structuredClone(s.rows)};}};
