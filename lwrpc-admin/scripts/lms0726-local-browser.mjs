...
const root=process.cwd(),dir=path.join(root,'..','.local-validation','lms0726-browser-fixture-'+Date.now());fs.mkdirSync(dir,{recursive:true});
...
await db.exec(fs.readFileSync(path.join(root,'supabase/migrations/20260909014356_lms0726_view_as_real_ui_reads.sql'),'utf8'));
...
const child=spawn(process.execPath,[path.join(root,'node_modules/next/dist/bin/next'),'dev','--webpack','--port','3076'],{cwd:dir,env,stdio:'inherit'});
