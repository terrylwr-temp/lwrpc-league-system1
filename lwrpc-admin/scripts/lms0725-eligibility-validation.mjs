...
if(process.argv.includes('--execute'))throw Error('Provider execution forbidden by cost policy for this runner');
const provider=false;
globalThis.fetch=()=>{throw Error('Network forbidden in deterministic eligibility benchmark');};
const {eligibilityIntent}=await import('../app/lib/aiEligibilityIntent.js');
const {runEligibility}=await import('../app/lib/aiEligibilityService.js');
const {catalog,sourceDatabase}=await import('../test/helpers/eligibilityFixture.mjs');
process.env.SUPABASE_SERVICE_ROLE_KEY='local-synthetic-benchmark-fixture';
...
const {clarificationFromRetrieval}=await import('../app/lib/aiConversation.js');
...
benchmark.cases.push(...JSON.parse(fs.readFileSync(new URL('../../docs/lms-0725-league-date-cases.json',import.meta.url))).cases);
benchmark.cases.push(...JSON.parse(fs.readFileSync(new URL('../../docs/lms-0725-age-reference-cases.json',import.meta.url))).cases);
benchmark.cases.push(...JSON.parse(fs.readFileSync(new URL('../../docs/lms-0725-q55-cases.json',import.meta.url))).cases.map(c=>({...c,expectedIntent:'rating_policy'})));
benchmark.cases.push(...JSON.parse(fs.readFileSync(new URL('../../docs/lms-0725-eligibility-cases.json',import.meta.url))).cases);
...
const phase=process.argv.includes('--family')?'family':'full';
if(phase==='family')benchmark.cases=[...benchmark.cases.filter(c=>['Q54','Q55','Q100','Q101','Q102','Q103'].includes(c.id)),...['When is my Season DUPR set?','When is my Season DUPR locked?','When does my Season DUPR lock?','Does my Season DUPR stay the same all season?','How long does my Season DUPR remain in effect?','Can my rating be reset during the season?'].map((question,i)=>({id:'P'+(i+1),question,expectedRoute:'DOCUMENT',expectedIntent:'rating_policy',expectedKind:'answer'}))];
const outputPath=new URL(`../../docs/lms-0725-eligibility-${phase}${only?'-'+only:''}-${provider?'model-results':'preflight'}.json`,import.meta.url);
...
for(const c of benchmark.cases){
 if(phase==='age'&&Number(c.id.slice(1))<90)continue;
 if(phase==='failed-first'&&!['Q21','Q45','Q46','Q47','Q64','Q65'].includes(c.id))continue;
 if(only&&c.id!==only)continue;
 const eligibility=eligibilityIntent(c.question);
 if(eligibility){
  let reads=0;const result=await runEligibility({body:{question:c.question},principal:{user:{id:'10000000-0000-4000-8000-000000000101'},receiptBinding:'synthetic',supabase:sourceDatabase()},loadCatalog:async()=>catalog,lookup:async()=>{reads++;return {data:{status:'success',rf:29,value:2.8,sourceIsNr:false}};},persist:async()=>true});
  assert.notEqual(result.kind,'technical_error',c.id);
  const expected=c.expectedRoute;
  assert.ok(eligibility.personal?expected.startsWith('HYBRID'):expected==='DOCUMENT',c.id);
  if(expected==='HYBRID_CLARIFICATION')assert.equal(result.kind,'clarification');
  if(expected==='HYBRID_UNAVAILABLE_CONTEXT'){assert.equal(result.eligibility.outcome,'CANNOT_DETERMINE');assert.equal(reads,0);}
  if(!eligibility.personal)assert.equal(reads,0);
  output.cases.push({id:c.id,question:c.question,route:expected,routePass:true,status:'deterministic_eligibility_pass',kind:result.kind,outcome:result.eligibility?.outcome,selfLookupCalls:reads,calls:0});persist();continue;
 }
 ...
 const clarification=clarificationFromRetrieval(resolution,r);
 if(clarification){assert.equal(c.expectedKind||'clarification','clarification');record.status='clarification_no_model';record.clarification=clarification.clarification;persist();continue;}
 try{
 ...
 if(answer.modelCallSkipped&&c.expectedKind!=='insufficient_evidence')throw new Error('Expected grounded document answer has no selected evidence.');
 ...
 if(c.expectedKind==='insufficient_evidence'){record.materialPass=answer.modelCallSkipped&&record.calls===0;}
 if(c.expectedKind==='answer'&&!c.expectedIntent){
 const text=answer.answer||'';
 record.requiredChecks=provider?[record.status==='generated',new RegExp(c.expectedMonth,'i').test(text),c.expectedDays.every(day=>new RegExp('\\b'+day+'\\b').test(text)),text.includes(String(c.expectedYear)),(answer.sources||[]).some(s=>/Important Dates/.test(s.documentTitle||s.title||''))]:[record.status==='preflight_pass'];
 record.materialPass=record.requiredChecks.every(Boolean);
 }
 if(c.expectedIntent==='rating_policy'){
  const text=answer.answer||'';
  record.requiredChecks=provider?[record.status==='generated',/captains/i.test(text),/before|prior/i.test(text),/(?:duration|entire|whole|all|throughout).{0,40}season|(?:remain|stay).{0,50}season/is.test(text),!(/never changes|never be reset|cannot ever/i.test(text)),(answer.sources||[]).some(s=>s.chunkId==='d51e615b-a4f2-460f-8a81-da9abfbd46af')]:[record.status==='preflight_pass'];
  record.materialPass=record.requiredChecks.every(Boolean);
 }
 if(c.expectedIntent&&c.expectedIntent!=='rating_policy'){
  const text=answer.answer||'';
  record.requiredChecks=provider?(c.expectedIntent==='age_eligibility'?[record.status==='generated',/December 31|Dec\.? 31/i.test(text),/current (?:calendar )?year|this year/i.test(text),/65/.test(text),!/(?:Oct\.?|October)\s*16/.test(text),(answer.sources||[]).every(s=>s.ruleNumber==='6.3')]:[record.status==='generated',/Oct(?:ober)?\.?\s*16/i.test(text),/2026/.test(text)]):[record.status==='preflight_pass'];
  record.materialPass=record.requiredChecks.every(Boolean);
 }
 ...
 if(record.status==='failed'||(provider&&(failedIds.has(c.id)||c.expectedKind)&&record.materialPass===false)){output.stoppedOn=c.id;persist();break;}
}
console.log(JSON.stringify({calls:output.calls,statuses:output.cases.reduce((a,c)=>(a[c.status]=(a[c.status]||0)+1,a),{})}));

if(output.stoppedOn)process.exitCode=1;

assert.equal(output.cases.length,120);assert.ok(output.cases.every(c=>c.routePass&&c.status!=='failed'));assert.equal(output.calls,0);
