...
const phase=process.argv.includes('--dates-only')?'dates':process.argv.includes('--failed-first')?'failed-first':'full';
const outputPath=new URL(`../../docs/lms-0725-league-dates-current-${phase}${only?'-'+only:''}-${provider?'model-results':'preflight'}.json`,import.meta.url);
...
for(const c of benchmark.cases){
 if(phase==='dates'&&Number(c.id.slice(1))<64)continue;
