...
const root=process.cwd(),dir=path.join(root,'..','.local-validation','lms0730-browser-fixture-'+Date.now());fs.mkdirSync(dir,{recursive:true});
...
const server=http.createServer(async(req,res)=>{res.setHeader('Access-Control-Allow-Origin','http://localhost:3092');res.setHeader('Access-Control-Allow-Headers',req.headers['access-control-request-headers']||'authorization,apikey,content-type');res.setHeader('Access-Control-Allow-Methods','GET,POST,PATCH,DELETE,HEAD,OPTIONS');res.setHeader('Content-Type','application/json');if(req.method==='OPTIONS'){res.end('{}');return;}
 try{
  ...
  if(['/rest/v1/rpc/ai_live_lookup','/rest/v1/rpc/ai_live_feedback'].includes(req.url)&&req.headers.apikey==='synthetic-service'){
   let raw='';for await(const chunk of req)raw+=chunk;const p=JSON.parse(raw);let result;
   await db.exec('set role service_role');try{result=req.url.endsWith('ai_live_lookup')?(await db.query('select public.ai_live_lookup($1,$2,$3) result',[p.p_actor,p.p_request,p.p_query])).rows[0].result:(await db.query('select public.ai_live_feedback($1,$2,$3,$4) result',[p.p_actor,p.p_answer,p.p_helpful,p.p_metadata])).rows[0].result;}finally{await db.exec('reset role');}res.end(JSON.stringify(result));return;
  }
  const url=new URL(req.url,'http://127.0.0.1:3093');
  ...
  if(url.pathname==='/rest/v1/line_games'&&['GET','HEAD'].includes(req.method)){const result=await queryNormalDashboardFixture(db,url.searchParams);res.setHeader('Access-Control-Expose-Headers','Content-Range');res.setHeader('Content-Range','0-0/'+result.count);res.end(req.method==='HEAD'?'':JSON.stringify(result.data));return;}
  if(url.pathname.startsWith('/rest/v1/')&&['GET','HEAD'].includes(req.method)){
   ...
   if(req.headers.accept?.includes('vnd.pgrst.object'))query=query.maybeSingle();const result=await query;if(result.error){res.statusCode=400;res.end(JSON.stringify(result.error));return;}res.setHeader('Access-Control-Expose-Headers','Content-Range');res.setHeader('Content-Range','0-0/'+(Array.isArray(result.data)?result.data.length:result.data?1:0));res.end(req.method==='HEAD'?'':JSON.stringify(result.data));return;
  }
...
const env={...process.env,NODE_ENV:'development',NEXT_TELEMETRY_DISABLED:'1',NEXT_PUBLIC_SUPABASE_URL:'http://127.0.0.1:3093',NEXT_PUBLIC_SUPABASE_ANON_KEY:'synthetic-anon',SUPABASE_SERVICE_ROLE_KEY:'synthetic-service',VIEW_AS_ENCRYPTION_KEY:Buffer.alloc(32,7).toString('base64'),LMS_ORIGIN:'http://localhost:3092',VIEW_AS_ORIGIN:'http://127.0.0.1:3092'};
...
const child=spawn(process.execPath,[path.join(root,'node_modules/next/dist/bin/next'),'dev','--webpack','--port','3092'],{cwd:dir,env,stdio:'inherit',windowsHide:true});
