-- LMS-0721 / 0.1.543: managed related-source passage binding only. Local validation before production review.
begin;
alter table public.ai_approved_answer_revisions add column if not exists related_passage text;
alter table public.ai_approved_answer_revisions drop constraint if exists ai_approved_related_binding;
alter table public.ai_approved_answer_revisions add constraint ai_approved_related_binding check (
 (related_chunk_id is null and related_passage is null and related_rule_identity is null)
 or (related_chunk_id is not null and related_passage is not null and related_rule_identity is not null
  and octet_length(related_passage) between 1 and 16384 and length(related_rule_identity)<=120));
-- The existing payload bound remains; include the binding in the total managed revision public byte budget.
alter table public.ai_approved_answer_revisions drop constraint if exists ai_approved_bound_public_bytes;
alter table public.ai_approved_answer_revisions add constraint ai_approved_bound_public_bytes check (
 octet_length(jsonb_build_array(title,topic_key,canonical_question,approved_answer,league_scope,temporal_scope,season_id,effective_on,expires_on,related_chunk_id,related_rule_identity,related_passage,public_links)::text)<=32768);
-- No backfill. Existing linked rows without a binding cause this migration to fail for review.
...
begin
 ...
 if p_action in ('create','save') then
  ...
  r.related_rule_identity:=p_body->>'related_rule_identity';r.related_passage:=p_body->>'related_passage';r.public_links:=coalesce(p_body->'public_links','[]'::jsonb);r.content_hash:=p_body->>'content_hash';
 end if;
 -- Recheck the retained container and exact text under the same transaction as save/activation.
 if p_action in ('create','save','activate') and r.related_chunk_id is not null then
  lock table public.ai_documents,public.ai_document_versions,public.ai_document_chunks in share mode;
  if not exists(select 1 from public.ai_document_chunks c join public.ai_documents d on d.active_version_id=c.document_version_id join public.ai_document_versions v on v.id=c.document_version_id
   where c.id=r.related_chunk_id and c.is_searchable and v.processing_status='ready' and d.status='active' and d.document_type<>'usap_rulebook'
    and r.related_passage is not null and position(r.related_passage in c.content)>0
    and (r.related_rule_identity='' or (r.related_rule_identity=c.rule_number and r.related_passage=btrim(c.content)) or left(ltrim(r.related_passage),length(r.related_rule_identity)+2)=r.related_rule_identity||'. ')) then
   raise exception 'approved_related_binding_invalid';
  end if;
 end if;
 if p_action='activate' then
  ...
  if r.related_passage is distinct from p_body->>'related_passage' or r.related_rule_identity is distinct from p_body->>'related_rule_identity' then raise exception 'approved_related_binding_changed'; end if;
  r.authority_manifest_hash:=manifest;
 ...
 if p_action in ('create','edit') then
  insert into public.ai_approved_answer_revisions(id,answer_id,revision_number,status,title,topic_key,canonical_question,approved_answer,league_scope,temporal_scope,season_id,effective_on,expires_on,related_chunk_id,related_rule_identity,related_passage,public_links,content_hash,created_at,updated_at,created_by_user_id,updated_by_user_id,row_version)
   values(r.id,r.answer_id,r.revision_number,r.status,r.title,r.topic_key,r.canonical_question,r.approved_answer,r.league_scope,r.temporal_scope,r.season_id,r.effective_on,r.expires_on,r.related_chunk_id,r.related_rule_identity,r.related_passage,r.public_links,r.content_hash,r.created_at,r.updated_at,r.created_by_user_id,r.updated_by_user_id,r.row_version);
 else
  update public.ai_approved_answer_revisions set title=r.title,topic_key=r.topic_key,canonical_question=r.canonical_question,approved_answer=r.approved_answer,
   league_scope=r.league_scope,temporal_scope=r.temporal_scope,season_id=r.season_id,effective_on=r.effective_on,expires_on=r.expires_on,related_chunk_id=r.related_chunk_id,related_rule_identity=r.related_rule_identity,related_passage=r.related_passage,
