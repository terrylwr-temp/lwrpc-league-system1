-- LMS-0725 / 0.1.547. LOCAL REVIEW ONLY.
begin;
do $guard$ declare p pg_catalog.pg_proc;begin
 select * into p from pg_catalog.pg_proc where oid='ai_live_private.lookup(uuid,uuid,jsonb)'::regprocedure;
 if pg_catalog.md5(p.prosrc) not in ('9e60d8bbd1342887d20d665ddf92b99b','3ac4c8bf5d0ed6845135df1206fdde51','fecb77efcf5c33008b43a230fc31d3e9') or p.proowner<>current_user::regrole or p.prosecdef or p.proconfig is distinct from array['search_path=""'] then raise exception 'Eligibility lookup drift';end if;
 if exists(select 1 from pg_catalog.aclexplode(coalesce(p.proacl,pg_catalog.acldefault('f',p.proowner))) a where a.grantee not in(p.proowner,'service_role'::regrole::oid) or a.is_grantable) then raise exception 'Eligibility ACL drift';end if;
 end $guard$;
...
revoke all on function ai_live_private.lookup(uuid,uuid,jsonb) from public,anon,authenticated;
...
do $guard$ declare p pg_catalog.pg_proc;begin
 ...
 if pg_catalog.md5(p.prosrc) not in ('79a9d17bdcf6a3fa34ca5dfc88b14944','a0ee5cd582179c95ba3d8deea9b89944','302293b776d734e1573238116c01ec4d') or p.proowner<>current_user::regrole or p.prosecdef or p.proconfig is distinct from array['search_path=""'] then raise exception 'Eligibility lookup drift';end if;
 if exists(select 1 from pg_catalog.aclexplode(coalesce(p.proacl,pg_catalog.acldefault('f',p.proowner))) a where a.grantee not in(p.proowner,'lms_view_as_executor'::regrole::oid) or a.is_grantable) then raise exception 'Eligibility ACL drift';end if;
 end $guard$;
...
revoke all on function view_as_private.lookup(uuid,uuid,uuid,uuid,jsonb) from public,anon,authenticated;
grant execute on function view_as_private.lookup(uuid,uuid,uuid,uuid,jsonb) to lms_view_as_executor;
do $guard$ declare p pg_catalog.pg_proc;begin select * into p from pg_catalog.pg_proc where oid='public.ai_live_lookup(uuid,uuid,jsonb)'::regprocedure;
 if pg_catalog.md5(p.prosrc) not in ('926b7c978757716cd99c0bce47686f19','2fd4659b8b0e4f9cd24b43b3168b9bec','0d064532a73f78ebb6de45e811d8fb88') or p.proowner<>current_user::regrole or p.prosecdef or p.proconfig is distinct from array['search_path=""'] then raise exception 'Eligibility wrapper drift';end if;
...
begin
 ...
 if result->>'status' in ('denied','not_found','rate_limited') and p_query->>'intent' in ('PLAYER_CONTACT','PLAYER_RATING','SELF_TEAM','TEAM_ROSTER','NEXT_MATCH','ELIGIBILITY_SELF')
...
do $guard$ declare definition text;begin select pg_catalog.pg_get_constraintdef(oid) into definition from pg_catalog.pg_constraint where conrelid='ai_live_private.access_audit'::regclass and conname='access_audit_intent_check' and convalidated and contype='c';
if definition is null or pg_catalog.md5(definition) not in ('cdca76d0afc5b1506a8ebb28f27ae710','72b0149951d344ef39f70b574c142795') then raise exception 'Eligibility audit constraint drift';end if;end $guard$;
grant select(dupr_reliability_rating,dupr_doubles_rating) on public.member_season_ratings to lms_view_as_executor;
