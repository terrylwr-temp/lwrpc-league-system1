...
The new approval requires the public club address not be treated as protected PII while explicitly prohibiting Stage 7 changes and requiring a stop on another genuine defect. Stopped before correcting either component or deploying. Recommend resolving whether the same reviewed exact-address exception may be used solely in Stage 7 email detection, retaining all other privacy checks and limits. No schema change is indicated. Existing `systemSettings.js` has main/support email defaults but also runtime overrides; a mutable support address should not silently become approved public knowledge. The implementation/configuration choice remains pending this stop.

No application code, tests, database, production environment or deployment changed in this pass. LMS-0721 / 0.1.543 remains deployed and not production accepted; scheduling lifecycle remains blocked. This diagnostic note is local documentation only.

## Current production acceptance checkpoint — 2026-09-06, after deployment 7b4aca4

**LMS-0721 / 0.1.543 is deployed, NOT production accepted.** This section supersedes the historical local/pending checkpoints below. The passage-binding migration and Existing Evidence workflow are now in production. Acceptance stopped at scheduling Draft validation: the exact owner-approved answer containing the official club email `info@lwrpickleballclub.com` is rejected with **“Do not include credentials or personal contact information.”** This previously reported restriction is unchanged. No policy rewrite or validator correction was made.

| Requested result | Evidence / status |
|---|---|
| 1. Preflight | Passed: correct production project, prior migrations present, passage migration absent, managed tables empty; Saturday case and current intentional corpus baseline verified before mutation. |
| 2. Passage-binding migration | Applied once, then verified column/byte constraints, managed RLS, service SELECT-only table access, denied browser access, service-only action RPC, safe search_path/timeouts. Previous full data/function/ACL checkpoint identical immediately after migration. Do not reapply. |
| 3. Deployment | Normal main pipeline, commit `7b4aca4fcfdc169114071f2e561a203de868f901`, READY deployment `dpl_B7isKuEVxuBRK1bF53bE4PLuriVH`; production alias and exact commit verified. Live footer LMS-0721, build 0.1.543 unchanged. |
| 4. Saturday evidence | Live source review separately displays Rule 6.2.2 — Roster & Courts — Page 9. Expanded passage explicitly permits additional players who participate only in the mixed round. |
| 5. Yes decision | Passed once: case New, category ai_retrieval_selection, revision 1 to 2, resolved_at NULL. One category_changed manager event at 2026-09-06 18:51:48.545935 UTC; validated document/version/chunk, Rule 6.2.2/page 9 and passage SHA-256 retained. No Approved Answer or fake linkage. |
| 6. UX / Retest | Live evidence and Yes/No/Close controls adjacent. At 390px viewport, page scroll width 379px; all three controls 44px high and within viewport. Retest opened Test AI Assistant with the exact Saturday question prefilled, no execution/output. Case return displays the audit and remains New. Local tests cover Close/focus, No continuation and preserved blocks. |
| 7. Website / NR | Live website case authority review displayed and expanded Rule 1.1/page 2 with the club website. Live manager source search for NR displayed Rule 4.5/page 3. Neither was saved. NR selected chunk contains only the start of 4.5; the previously documented adjacent-chunk authority review establishes the complete rule. Do not claim this picker alone proves automatic duplicate blocking of every NR wording. |
| 8. Rule 5.11 picker | Passed live: distinct 5.10 and 5.11 options; selecting 5.11 displays exact Rescheduling & Score Submission Deadlines provision, page 5, including same-week/Sunday limits. Application exact-binding validator and isolated persistence/replay tests passed previously. Production Draft persistence remains unproven because validation stops before save. |
| 9. Manager-originated Draft | BLOCKED: entered the exact approved policy, All/Standing/no expiration and Rule 5.11; Save Draft returned the public-email validation error. Read-only verification confirms zero answers, revisions and managed audit events. |
| 10. Draft isolation | Pending production lifecycle; no Draft exists. Existing isolated coverage retained. |
| 11. Activation | Pending; no activation attempted. |
| 12. Canonical / natural retrieval | Pending Active-policy acceptance; no policy was introduced into player retrieval. |
| 13. Viewer | Pending production Approved Answer revision/viewer acceptance. |
| 14. Feedback provenance | Pending; no acceptance feedback event created. |
| 15. Revision lifecycle | Pending; no artificial policy revision created. |
| 16. Historical revision | Pending production lifecycle. |
| 17. Retirement | Pending production lifecycle. |
| 18. Authority Warning | Existing isolated/automated coverage passed in the 467-test suite; no production conflict manufactured. |
| 19. Case-originated regression / idempotency | Existing isolated PostgreSQL coverage preserves populated occurrence, open state and managed counts; same-operation replay appends no second event. Production Yes was not repeated. Genuine case-origin lifecycle remains covered by existing automated tests; no fake case created. |
| 20. LMS-0720 sanity | Stage 7B page loaded successfully. Fresh live usin/volly/comunity/medical follow-up/protected typo/New Question execution not run after the scheduling stop; earlier automated regressions remain, but are not a fresh live acceptance pass. |
| 21. Integrity | Final checkpoint: unchanged full-row corpus hashes (7 documents/20 versions/1,581 chunks), feedback 16, outcomes 105, occurrences 24, groups/routes 21. HMAC configuration not accessed or changed; route hash and key version 1 unchanged. Cases remain 18; manager events 24 to 25 through the single authorized Saturday action. Existing activation/review/capture/Stage 3 RPC definitions and ACL hashes unchanged. Managed tables remain 0/0/0. |
| 22. Limitations / deferred | Public club-email validation blocks scheduling lifecycle. PDF authoritative activation date/time/actor remains an accepted deferred enhancement, not a blocker. Saturday retrieval, website selection, rally-scoring scope, Live LMS Intelligence and other deferred quality work remain untouched. |
| 23. Final status | DEPLOYED, PRODUCTION ACCEPTANCE BLOCKED pending separately approved public-email correction and remaining live lifecycle gates. No new version started. |

Saturday original answer ID `b23b01e9-74fb-454d-ad26-0c9ab32a807a`; occurrence full-row MD5 remains `c849fee5681ac9a777876e09308062e9`. The new audit references active Rules version `c0604ad8-7057-4e63-b6e1-e9389aee2157`, chunk `e4bebbb3-7a89-42c6-ab33-dfb18baad0c3`, passage SHA-256 `9b536331bea8e4f4e94e8770edf5cd00584e3a9eb02547e2d07b919e9235bf95`. No member identifiers are included in this report.

Validation basis remains the exact deployed code's 467 passing tests, lint with six existing warnings, TypeScript/PDF checks and successful isolated production build following the known post-compilation cache lock. No application code changed during acceptance; final documentation update is local only. The blocked production form was left available for owner review; it is unsaved.
...
## CURRENT production checkpoint — 2026-09-06 (941fd96)

**DEPLOYED, NOT PRODUCTION ACCEPTED.** Name/public-contact correction is READY in deployment dpl_7CbgkUaeA12Y6Eigqa2HVwhvNkL9, exact commit 941fd96fd6c9188ec480423e1b8fab6eb1b9b117, production alias verified. No migration. Local validation: 473 tests, lint six existing warnings, nonincremental TypeScript/PDF pass, normal compilation then known cache lock, isolated 74-page build pass.

Exact owner policy saved unchanged as manager-created Draft: answer 8d11bdd6-5024-416a-ab48-df90a5b35477, revision a9880b0e-da01-4692-aedf-020f07b22bf7 (revision 1). NULL source_review_case_id, one creation audit, exact 5.11 passage retained. Draft canonical and natural-variant requests returned existing insufficient evidence without citing/using Draft. No Draft viewer token was issued; isolated unauthorized-Draft viewer checks remain the proof of access denial. Activation succeeded at 2026-09-06 19:21:34.828870 UTC with actor/embedding present and one activation audit. One answer/revision and two managed events now exist.

Canonical Active answer passed at answer_id 08baea09-899d-4138-979e-8b5b2f97d4a3: normal grounded answer, mutual agreement/same-week scheduling and notification email retained, Approved Answer revision 1 cited, no Authority Warning. Viewer displays exact unchanged revision text and exact related Rules 5.11/page 5 passage; manager/audit fields absent. Formal 5.11 was available in the related viewer, not separately selected as a second source in the canonical answer. The viewer displayed the bound passage; a separate PDF-open link was not demonstrated.

One controlled Helpful event at 2026-09-06 19:23:03.081149 UTC is correlated to that answer and exact revision/content hash 91809791df17409722e4baf3034c32cc698e9dc3a9dffbf60ce4ba5204269f83. Generated answer retains info@lwrpickleballclub.com. Stage 7 linked occurrence redaction_applied=false; exact revision provenance retained; binding is recoverable through that immutable revision. No signed URLs, credentials or extra member data in inspected snapshots. Approved text preserves Rule's; generated answer naturally omits that phrase, so full exact-text privacy preservation is proven locally, not claimed as verbatim model output.

**STOP at Active natural variant:** “Can we move our match to another day?” still returns insufficient_evidence, answer_id dd1cda34-b0e6-4531-b4fb-c072746d304f, completed 2026-09-06 19:24:03.878 UTC. Retained diagnostics: Stage 3 sufficient, 32 candidates, threshold .35; Rule 5.11-containing chunk 01444d6d-44da-4fd9-8069-e7fe1e117e28 rank 1 score .4154; selectedEvidence empty. No inference made about the managed-search cause from that bounded snapshot. Separate diagnosis/authorization is needed before retrieval correction. Revision 1 remains ACTIVE; no new revision/replacement/retirement attempted after stop. Historical revision/retirement, remaining live role/URL nuances and LMS-0720 live sanity remain pending. Existing isolated authority/case/privacy regression suite passes. No artificial conflict or fake case created.

Pre-lifecycle baseline included intervening legitimate activity (108 outcomes/27 occurrences/22 groups/19 cases/28 review events). After four authorized player questions and one feedback event: 112 outcomes/31 occurrences/25 groups/21 cases/31 review events, 17 feedback events. Prior feedback hash remains cb2a34ba555949c2631fb02a8febf1d5; Saturday original occurrence hash remains c849fee5681ac9a777876e09308062e9. Corpus remains 7/20/1581 with identical hashes. HMAC configuration untouched; route key version 1 retained. No backfill or historical rewrite.

Additional owner-requested Existing Official Evidence Confirmed copy is implemented locally with the exact explanatory text and unchanged Retest handler. Lint/diff and isolated compile pass (apostrophe escaped without display change). It is NOT in deployed 941fd96: no further deployment after the new lifecycle stop. Saturday 6.2.2 is recorded as future AI quality diagnosis. PDF activation date/actor display remains deferred, not a blocker. No new LMS version.
...
## Shared public-contact correction — local regression stop (2026-09-06)

Implemented the authorized shared exact-address classifier in `publicOrganizationalContacts.js`, used by Draft validation and Stage 7 email detection. Only info@lwrpickleballclub.com (case-insensitive) is public; formatting is preserved. Runtime support settings cannot enlarge the reviewed allowlist. Unapproved club-domain/personal/third-party addresses remain rejected/redacted. Draft validation retains URL protections and explicitly rejects secret/token/Supabase/HMAC-key wording and private phone patterns required by the approval. Stage 7's other filters are unchanged. No schema/backfill/rendering/layout change.

**STOP before deployment:** the full exact-policy regression exposes an additional existing Stage 7 name-filter false positive. `redactQualityText("subject to the Rule's scheduling and score-reporting requirements.",6000)` returns `[detail omitted for privacy]`, redacted true. The pattern for capitalized possessive names matches `Rule's`. Independently, `Notify info@lwrpickleballclub.com.` now remains intact with redacted false. The public-email correction therefore works, but preserving the exact scheduling policy in that snapshot helper would require a separate bounded name-filter decision. No name-filter change or policy rewrite was made. Grounded-feedback generated-answer storage is separate; this finding is specifically the Stage 7 redaction helper and does not assert that all feedback answer storage is lost.

Validation: full npm test 471 total / 470 pass / 1 fail (new exact-policy snapshot assertion); existing regressions pass. Lint: six existing warnings, no errors. Nonincremental TypeScript and PDF server bundle verification pass. Normal build compiled in 11.8s then hit the known .next/cache/.tsbuildinfo EPERM; no isolated build was pursued after the genuine regression stop. git diff --check passes. Changes remain local and undeployed. No production Draft, activation, migration or acceptance interaction in this pass. Version remains LMS-0721 / 0.1.543, deployed baseline 7b4aca4, NOT production accepted. Failing regression retained for the next authorized correction.

## Name-classification correction validated — 2026-09-06

Root cause: redactQualityText only replaces control characters with spaces and trims. It performs no name tokenization, dictionary lookup or apostrophe normalization. The old capitalized-word possessive branch matches both Rule's and Rule’s directly, setting sensitive=true and omitting the whole field. Plain Rule/Rules/Rule 5.11 and plural Rules' do not match; Rules's does. The new helper extracts the same possessive candidates and excludes only bounded structural spans: an actual official rule/role phrase at field start or after the/under, followed by a structural object such as requirements, scheduling, roster or format. Explicit player/member/named and eligibility identity contexts run first on original text; other possessive names elsewhere still cause whole-field omission. Formatting is not normalized or rewritten. John Rule's remains protected; no blanket rule-word or capitalization bypass.

Bounded constructed-prose scan: possessive Rule, Rules, League, Management, Captain, Home Captain, Visiting Captain, Team, Division, Match Setup, USA Pickleball and Picklebreaker all exhibited the same old capitalized final-token false positive in “the [term]'s requirements”. Season DUPR did not (uppercase acronym excluded by the original pattern). These documented domain phrases now pass only in structural context; not globally whitelisted. The production-format scheduling policy plus existing test corpus remain regression inputs. No personal-name dictionary or broader privacy redesign added. Contact John Smith with personal email and existing member/possessive controls remain redacted; arbitrary names without existing heuristic context are not newly claimed covered.

Shared publicOrganizationalContacts.js classifies only the reviewed info address case-insensitively. Both managed validation and Stage 7 use it; runtime settings cannot expand it. Exact/curly policy, display casing, spoof/unapproved domains, mixed names/email, phone/credentials/tokens/signed URLs and snapshot length bounds tested. Player text rendering is unchanged and plain text; no mailto injection introduced. No history/backfill, schema, HMAC, triggers, outcomes, grouping, retention, corpus, retrieval or layout change.

Validation: full npm test 473/473 passed. Lint six existing warnings, zero errors. Nonincremental tsc/PDF bundle/diff checks pass. Normal build compiled in 13.5s then existing tsbuildinfo EPERM; isolated clean build compiled in 11.2s, TypeScript and all 74 pages/optimization passed. A test-only initial chr typo was corrected before the final full run. This supersedes prior pre-edit stops; authorized deployment follows. LMS-0721 / 0.1.543 unchanged; not production accepted until live lifecycle completes.

## Additional manager UX clarification (2026-09-06)

Owner-authorized copy-only follow-up: post-Yes confirmation now says Existing Official Evidence Confirmed and explicitly explains that classification does not fix Ask LWR, retrieval/selection must be corrected first, then Retest and explicit Resolve. Retest handler, state mutation and semantics are unchanged. Saturday mixed-doubles question with manager-confirmed Rule 6.2.2 is a future bounded AI quality diagnosis item; not missing knowledge and no Approved Answer should be created for it. No retrieval correction in LMS-0721.

## Natural-variant diagnosis (2026-09-06)
...
**Production follow-up stop:** correction deployed as `a3e4923`, Vercel `dpl_5ochKYHXsWWC9pQ4QM99utqmbCV1` READY. First live reschedule question selected formal 5.11 plus revision 1 and returned `answer`, but omitted the supplement's notice-for-every-change obligation. Source label also inherited the 5.10 container's Video Recording heading. Stopped before remaining live variants or lifecycle actions. See the full scheduling implementation report for exact output and outcome. No further code change/deployment. Revision 1 and .65 unchanged; NOT production accepted.

Subsequent approved implementation: see [scheduling implementation and validation](lms-0721-scheduling-implementation.md). Bounded applicability, exact server-revalidated related passage and manager diagnostics implemented; .65/revision 1 unchanged. 499 tests and required checks passed, with isolated clean build following the known cache lock. Authorized production deployment and >=.65 acceptance results are tracked in that report; below-threshold recall and revision lifecycle remain separate.

See [complete diagnosis](lms-0721-scheduling-binding-diagnosis.md). Reused prior scores without new embeddings or generation. Exact passing-score variant is `Can we reschedule our match?` (.6849): 5.11 rank 2/.4723 fails `reschedule` versus `rescheduled`; scoped 6.1.9.5/6 passes literal matching, leaving the wrong chunk selected and the supplemental binding gate unsatisfied. Canonical selected revision 1 alone, not independently bound formal evidence. The exact variant's final generated kind is unmeasured; its formal-only selection must not be confused with the observed insufficient-evidence response for the move-another-day question.

Actual predicate probes confirmed all seven positives fail the current exact 5.11 matcher. Read-only production checks reconfirmed 5.11/5.12, interrupted-play/flex restrictions, and unchanged full revision hash. Recommended bounded independent topic/proposition applicability and exact-passage binding; no implementation or threshold change. Three below-.65 positives still require later recall evaluation. No schema/migration required for the proposed formal correction. Stage 7 records, Active revision 1, public email, historical provenance and pending UX copy preserved. Documentation diff check passed. LMS-0721 remains NOT production accepted; stop for review.
