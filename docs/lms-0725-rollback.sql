-- LMS-0725 review-only rollback. Requires explicit release authorization.
begin;
do $guard$ begin
 if (select pg_catalog.md5(prosrc) from pg_catalog.pg_proc where oid='ai_live_private.lookup(uuid,uuid,jsonb)'::regprocedure)<>'9e60d8bbd1342887d20d665ddf92b99b' then raise exception 'LMS-0725 rollback drift';end if;
 end $guard$;
...
do $guard$ begin
 if (select pg_catalog.md5(prosrc) from pg_catalog.pg_proc where oid='view_as_private.lookup(uuid,uuid,uuid,uuid,jsonb)'::regprocedure)<>'79a9d17bdcf6a3fa34ca5dfc88b14944' then raise exception 'LMS-0725 rollback drift';end if;
 end $guard$;
