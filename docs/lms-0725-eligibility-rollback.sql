-- Review only. Preserve eligibility audit history and its allowlist.
begin;
do $guard$ begin if (select pg_catalog.md5(prosrc) from pg_catalog.pg_proc where oid='ai_live_private.lookup(uuid,uuid,jsonb)'::regprocedure)<>'fecb77efcf5c33008b43a230fc31d3e9' then raise exception 'Eligibility rollback drift';end if;end $guard$;
...
do $guard$ begin if (select pg_catalog.md5(prosrc) from pg_catalog.pg_proc where oid='view_as_private.lookup(uuid,uuid,uuid,uuid,jsonb)'::regprocedure)<>'302293b776d734e1573238116c01ec4d' then raise exception 'Eligibility rollback drift';end if;end $guard$;
...
do $guard$ begin if (select pg_catalog.md5(prosrc) from pg_catalog.pg_proc where oid='public.ai_live_lookup(uuid,uuid,jsonb)'::regprocedure)<>'0d064532a73f78ebb6de45e811d8fb88' then raise exception 'Eligibility wrapper rollback drift';end if;end $guard$;
