-- REVIEW-ONLY rollback: restores the known-denying prior behavior. Never auto-run.
begin;
do $$begin
 if (select md5(prosrc) from pg_proc where oid='public.lms_view_as(text,jsonb)'::regprocedure)<>'77ef613cac6db4df5e6849a686be0c26' then raise exception 'Unexpected dispatcher: stop rollback';end if;
end $$;
...
create or replace function public.lms_view_as(p_op text,p_input jsonb) returns jsonb language plpgsql security definer set search_path='' as $prior$
