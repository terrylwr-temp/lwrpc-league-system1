...
Date: 2026-09-06. **Governing design approved; implementation authorized, including the subsequent Authority Warning workflow clarification.** Current production is **LMS-0721 / 0.1.543, deployed but not production accepted**. LMS-0720 / 0.1.542 is the last accepted release; Stage 7A/B remain accepted. The manager-origin corrective migration is applied and application commit f30927b is deployed READY. Production acceptance stopped before Draft creation: the scheduling source picker offers Rule 5.10 instead of the required Rule 5.11. The stored chunk contains both rules; source-review fallback and activation identity use the broader stored label. No correction has been made after this stop. PDF activation history remains deferred and latest acceptance gate 23 unmet. See the implementation report final continuation matrix.
...
Latest approved passage-binding implementation (2026-09-06): add only related_passage text to managed revisions (16 KiB byte bound), retain existing related_rule_identity through Draft/save/edit, and include both in the content hash and existing audit revision state. Exact document/version remain resolved via the retained chunk FK chain. Server validation enumerates trusted structural provisions/families and uses the LMS-0719 identity utility's explicit managedSibling mode; ordinary citation mode is unchanged. Activation revalidates the binding and current source; historical viewer reloads the original container and exact passage. Migration 20260906180112 is local only, awaiting review before production application/deployment; 457 tests and isolated production build pass. See implementation report for security proof, exact continuation and separate existing policy-email rejection. PDF activation timestamp/actor history is expressly deferred and is not a blocker for this correction, per latest approval.

2026-09-06 related-source assessment: the approved application correction reached its explicit schema stop. The current revision schema/action RPC cannot durably retain a manager-selected passage: create/save clear related_rule_identity and retain only related_chunk_id. Proposed, not implemented: a bounded selected-passage binding on the managed revision, derived identity retained through Draft/edit, content-hash inclusion and activation revalidation against the exact chunk/version/document. The shared LMS-0719 utility also needs bounded trusted sibling-provision handling for stored 5.10 / selected 5.11. See the implementation report's schema-stop section. No migration has been written or applied for this finding.
