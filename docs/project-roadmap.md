...
- Forgot Password email validation now uses a server route for member checks when `SUPABASE_SERVICE_ROLE_KEY` or `SUPABASE_SERVICE_KEY` is configured. If no service key is available, it avoids false member-not-found errors from anonymous RLS and proceeds with Supabase's reset email flow after local email-format validation.
