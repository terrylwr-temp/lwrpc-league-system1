...
- PBCC final match-night results now always use SMS delivery for players with phone numbers, bypassing app-notification preferences for both automatic completion sends and manual result resends. Other PBCC messages retain their existing push-first behavior. Partner Comparison names now open a filtered regular-match play-history popup with dates played, last played, record, win percentage, and point differential, and the comparison header explains that player names are clickable. The visible app version was bumped to `LMS-0622`.
