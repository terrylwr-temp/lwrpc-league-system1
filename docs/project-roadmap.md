...
- Tournament Public Display expanded mode can now be exited with the Escape key, restoring the display header and its controls while returning the display to the top. Court Detail and Courts Only now render the `vs` separator in contrasting blue text for better distance viewing. The visible app version was bumped to `LMS-0674`.
