...
- Dashboard Appearance now offers a browser-saved collapsed desktop sidebar that expands on hover or keyboard focus without affecting mobile layout. Captain Match Setup shows individual rating range alongside the doubles-team maximum, uses one row for the two team-status pills with a full-width setup action, result details show entered/verified score audit names, and standings summaries show only the top five teams while full standings popups explain the division's configured tiebreak order. Captain team-roster dialogs now layer above match-detail dialogs. The visible app version was bumped to `LMS-0534`.
