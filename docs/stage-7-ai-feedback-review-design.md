...
## LMS-0718 / Stage 7B implementation checkpoint — 2026-09-05

The owner approved `lms-0718-stage7b-readiness-report.md` as the governing initial implementation scope. LMS-0717 is production accepted; LMS-0718 / 0.1.540 is implemented locally, pending migration/deployment review and production acceptance. The initial page uses Needs Review, Unanswered, Feedback and Resolved, with conflicts prominent in Needs Review. Manual merge/split and assignment tooling remain deferred; the earlier full Stage 7 design remains the future reference. No backfill or retention execution is included.

The function-only migration adds `ai_review_case_action`, `ai_review_feedback_state`, `ai_review_group_rows` and `ai_review_report`. All use invoker rights and a fixed search path, with EXECUTE restricted to service_role. Existing tables, capture RPC and table/default privileges are unchanged. Manager HTTP authorization remains Commissioner/League Manager only. The mutation locks group then case, matching capture lock order, updates case state and appends its audit event atomically. Revision checks and operation-ID retries protect concurrent edits. The server signs actor-bound review cutoffs from displayed retained activity, not wall-clock time; later activity remains outstanding.

Reporting excludes manager tests/protected/clarification from unanswered metrics and keeps legacy parentless feedback visible without fabricated outcomes. Missing historical diagnostics are labeled unavailable. Historical source viewing is a separate manager-only path; the accepted player viewer remains unchanged. Capture health displays Unknown or Degraded according to the existing endpoint, never inferring Recording from a recent row. Full implementation/validation and deployment acceptance steps are in `lms-0718-implementation-report.md`.
# LMS-0718 controlled production checkpoint — 2026-09-05

The approved four-function migration and LMS-0718 / 0.1.540 application are deployed. Production acceptance is **blocked**, so Stage 7B is not complete. A retained legacy Rules version is `superseded` and its exact chunk still exists/matches, but the manager source-link handler requires `ready` and returns Historical source unavailable. No current version was substituted. No correction was made; owner review is required before changing that guard and resuming remaining acceptance. See the LMS-0718 implementation report for the full 20-gate result and unchanged historical data/schema hashes. One authorized community case completed the six-action audit workflow and remains Resolved. No next stage/version or Live LMS Intelligence started.


## LMS-0718 final corrected acceptance — 2026-09-06 UTC

**LMS-0718 / 0.1.540 PRODUCTION ACCEPTED; Stage 7B COMPLETE.** Supersedes the initial blocked checkpoint above. The approved manager-only historical source correction permits exactly ready/superseded version states after binding to the retained snapshot document/version/chunk; processing/failed/missing/mismatched and unrelated identities remain denied. Historical Source — Superseded labeling distinguishes retained evidence from current governing content. No player source policy or Stage 7A/workflow/schema changes. Live historical PDF page 3, USAP/protected sanity, remaining navigation/accessibility and unchanged historical data verification passed. Full correction suite: 317 tests, lint six existing warnings, tsc/PDF/diff checks and isolated production build passed. Unavailable-role isolated evidence and no production fault/load injection remain accepted limitations. No next stage/version or Live LMS Intelligence started; future New Question remains documentation only.
